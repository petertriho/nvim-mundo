...
function assert.not_equals(actual, expected, message)
    if actual == expected then
        error(string.format("%s\nExpected not to equal: %s", message or "Assertion failed", vim.inspect(expected)))
    end
end

function assert.is_true(value, message)
    if value ~= true then
        error(string.format("%s\nExpected: true\nActual: %s", message or "Assertion failed", vim.inspect(value)))
    end
end
...
function assert.is_not_nil(value, message)
    if value == nil then
        error(string.format("%s\nExpected: not nil\nActual: nil", message or "Assertion failed"))
    end
end
...
function assert.throws(fn, expected_error, message)
    ...
    if success then
        error(string.format("%s\nExpected function to throw an error", message or "Throws assertion failed"))
    end
