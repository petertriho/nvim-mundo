...
_G.vim.fn.bufnr = function(name)
    if name == "__Mundo__" then
        return 2
    end
    if name == "__Mundo_Preview__" then
        return 3
    end
    return -1
end
_G.vim.fn.bufwinnr = function(bufnr)
    if bufnr == 2 or bufnr == 3 then
        return 1
    end
    return -1
end
...
test.it("should check if Mundo is visible", function()
    ...
    _G.vim.fn.bufwinnr = function(bufnr)
        if bufnr == 2 then
            return 1
        end -- Mundo window exists
        if bufnr == 3 then
            return -1
        end -- Preview window doesn't exist
        return -1
    end
...
test.it("should extract target state from line", function()
    ...
    _G.vim.fn.bufnr = function(name)
        return name == "__Mundo__" and 2 or -1
    end
    _G.vim.fn.bufwinnr = function(bufnr)
        return bufnr == 2 and 1 or -1
    end
    _G.vim.fn.win_getid = function()
        return 100
    end
...
test.it("should cleanup Mundo buffers", function()
    ...
    _G.vim.fn.bufloaded = function(bufnr)
        return bufnr == 2 or bufnr == 3
    end
