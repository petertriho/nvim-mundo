...
.PHONY: test test-unit test-integration test-config test-utils test-node test-tree test-graph test-symbols test-search clean format help
...
help:
	...
	@echo "  test-search    - Run search module tests"
...
test-unit:
	...
	@LUA_PATH="$(LUA_PATH)" $(NVIM_CMD) "lua require('tests.run_tests')" -c "qa!" tests.test_search
